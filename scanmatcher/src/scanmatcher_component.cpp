  ...
  broadcaster_(this)
{
  ...
  if (registration_method_ == "NDT") {
    ...
    pclomp::NormalDistributionsTransform<pcl::PointXYZRGB, pcl::PointXYZRGB>::Ptr
      ndt(new pclomp::NormalDistributionsTransform<pcl::PointXYZRGB, pcl::PointXYZRGB>());
    ...
    registration_ = ndt;
	  ...
	  boost::shared_ptr<pclomp::GeneralizedIterativeClosestPoint<pcl::PointXYZRGB, pcl::PointXYZRGB>>
      gicp(new pclomp::GeneralizedIterativeClosestPoint<pcl::PointXYZRGB, pcl::PointXYZRGB>());
...
void ScanMatcherComponent::convertPointCloud2ToPCL(
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr& output)
{
  ...
  for (size_t i = 0; i < input->width * input->height; ++i) {
    const uint8_t* ptr = &input->data[i * input->point_step];
    
    // Read XYZ
    output->points[i].x = *reinterpret_cast<const float*>(ptr + 0);
    output->points[i].y = *reinterpret_cast<const float*>(ptr + 4);
    output->points[i].z = *reinterpret_cast<const float*>(ptr + 8);
    
    // Read RGB as floats and convert to uint8
    float r_float = *reinterpret_cast<const float*>(ptr + 12);
    float g_float = *reinterpret_cast<const float*>(ptr + 16);
    float b_float = *reinterpret_cast<const float*>(ptr + 20);
    
    // Convert float values to uint8 (assuming the floats represent 0-255 range)
    uint8_t r = static_cast<uint8_t>(std::min(255.0f, std::max(0.0f, r_float)));
    uint8_t g = static_cast<uint8_t>(std::min(255.0f, std::max(0.0f, g_float)));
    uint8_t b = static_cast<uint8_t>(std::min(255.0f, std::max(0.0f, b_float)));
    
    // Pack RGB into the PCL point
    uint32_t rgb = ((uint32_t)r << 16 | (uint32_t)g << 8 | (uint32_t)b);
    output->points[i].rgb = *reinterpret_cast<float*>(&rgb);

    // // Debug output for first few points
    // if (i < 5) {
    //   RCLCPP_INFO(get_logger(), "Point %zu: XYZ=(%.3f, %.3f, %.3f), RGB float=(%.1f, %.1f, %.1f), RGB uint8=(%u, %u, %u)",
    //               i, output->points[i].x, output->points[i].y, output->points[i].z,
    //               r_float, g_float, b_float, r, g, b);
    // }
  }
}
...
void ScanMatcherComponent::convertPCLToPointCloud2(
    ...
    sensor_msgs::msg::PointCloud2::SharedPtr& output)
{
    ...
    output->fields[3].name = "r";
    output->fields[4].name = "g";
    output->fields[5].name = "b";
    ...
    // Set offsets
    output->fields[0].offset = 0;  // x
    output->fields[1].offset = 4;  // y
    output->fields[2].offset = 8;  // z
    output->fields[3].offset = 12; // r
    output->fields[4].offset = 16; // g
    output->fields[5].offset = 20; // b

    // Set basic message properties
    output->height = input->height;
    output->width = input->width;
    output->is_bigendian = false;
    output->point_step = 24;  // 6 fields * 4 bytes each
    ...
    for (size_t i = 0; i < input->points.size(); ++i) {
        size_t offset = i * output->point_step;
        const auto& point = input->points[i];

        // Copy XYZ
        memcpy(&output->data[offset + 0], &point.x, sizeof(float));
        memcpy(&output->data[offset + 4], &point.y, sizeof(float));
        memcpy(&output->data[offset + 8], &point.z, sizeof(float));

        // Unpack RGB
        uint32_t rgb = *reinterpret_cast<const uint32_t*>(&point.rgb);
        float r = static_cast<float>((rgb >> 16) & 0xFF);
        float g = static_cast<float>((rgb >> 8) & 0xFF);
        float b = static_cast<float>(rgb & 0xFF);

        // Copy RGB as separate floats
        memcpy(&output->data[offset + 12], &r, sizeof(float));
        memcpy(&output->data[offset + 16], &g, sizeof(float));
        memcpy(&output->data[offset + 20], &b, sizeof(float));
    }
}
...
void ScanMatcherComponent::initializePubSub()
{
  ...
  auto cloud_callback =
    [this](const typename sensor_msgs::msg::PointCloud2::SharedPtr msg) -> void
    {
      ...
      convertPointCloud2ToPCL(msg, tmp_ptr);
      ...
      if (use_min_max_filter_) {
        double r;
        pcl::PointCloud<pcl::PointXYZRGB>::Ptr tmp_ptr2(new pcl::PointCloud<pcl::PointXYZRGB>());
...
void ScanMatcherComponent::initializeMap(const pcl::PointCloud <pcl::PointXYZRGB>::Ptr & tmp_ptr, const std_msgs::msg::Header & header)
{
...
void ScanMatcherComponent::receiveCloud(
  const pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr & cloud_ptr,
  ...
  if (mapping_flag_ && mapping_future_.valid()) {
    ...
    if (status == std::future_status::ready) {
      if (is_map_updated_ == true) {
        pcl::PointCloud<pcl::PointXYZRGB>::Ptr targeted_cloud_ptr(new pcl::PointCloud<pcl::PointXYZRGB>(
            targeted_cloud_));
        if (registration_method_ == "NDT") {
          registration_->setInputTarget(targeted_cloud_ptr);
        } else {
          pcl::PointCloud<pcl::PointXYZRGB>::Ptr filtered_targeted_cloud_ptr(
            new pcl::PointCloud<pcl::PointXYZRGB>());
          pcl::VoxelGrid<pcl::PointXYZRGB> voxel_grid;
  ...
  const geometry_msgs::msg::PoseStamped current_pose_stamped)
{
  ...
  for (int i = 0; i < num_targeted_cloud_ - 1; i++) {
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr transformed_tmp_ptr(new pcl::PointCloud<pcl::PointXYZRGB>());
...
{
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr map_ptr(new pcl::PointCloud<pcl::PointXYZRGB>);
  for (auto & submap : map_array_msg.submaps) {
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr submap_cloud_ptr(new pcl::PointCloud<pcl::PointXYZRGB>);
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr transformed_submap_cloud_ptr(
        new pcl::PointCloud<pcl::PointXYZRGB>);
  ...
  convertPCLToPointCloud2(map_ptr, map_msg_ptr);
  map_msg_ptr->header.frame_id = map_frame_id;


  // RCLCPP_INFO(get_logger(), "PointCloud2 message after pcl::toROSMsg:");
  //   RCLCPP_INFO(get_logger(), "Height: %d, Width: %d, Point step: %d, Row step: %d", 
  //               map_msg_ptr->height, map_msg_ptr->width, 
  //               map_msg_ptr->point_step, map_msg_ptr->row_step);
    
  //   for (const auto& field : map_msg_ptr->fields) {
  //       RCLCPP_INFO(get_logger(), "Field: %s", field.name.c_str());
  //       RCLCPP_INFO(get_logger(), "  Offset: %d", field.offset);
  //       RCLCPP_INFO(get_logger(), "  Datatype: %d", field.datatype);
  //       RCLCPP_INFO(get_logger(), "  Count: %d", field.count);
  //   }

  //   // You might also want to check the actual data for the first few points
  //   if (map_msg_ptr->data.size() >= map_msg_ptr->point_step) {
  //       RCLCPP_INFO(get_logger(), "First point data:");
  //       for (size_t i = 0; i < map_msg_ptr->point_step; i += 4) {
  //           float value = *reinterpret_cast<const float*>(&map_msg_ptr->data[i]);
  //           RCLCPP_INFO(get_logger(), "  Offset %zu: %f", i, value);
  //       }
  //   }
